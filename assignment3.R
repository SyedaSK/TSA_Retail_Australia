...
plot(retail_data, type='o',ylab="Department stores retail", main=" Fig 1:Time Series plot of total number of department stores in retail ")
plot(retail_data,type='l', ylab = "Number of department stores", main="Fig 1:Time series plot of total number of department stores in retail.")
...
acf(as.vector(retail_data), main = "Figure 3: ACF of number of department stores in retail", xlab = "Lag", ylab = "ACF", lag.max=50)
pacf(as.vector(retail_data), main = "Figure 4: PACF of total number of department stores in retail", xlab = "Lag", ylab = "PACF",ylim = c(-1, 1))

# first lag scatter plot
lag1_retail_data <- zlag(retail_data)
index = 2:length(retail_data)
plot(y=retail_data,x=lag1_retail_data, ylab='Number of stores', xlab='First lag', main= "Figure 5: Scatter plot of neighboring number of stores")
...
plot(y=retail_data,x=lag2_retail_data, ylab='Number of stores', xlab='Second lag', main= "Figure 6: Scatter plot of number of stores and its second lag values")
cor(retail_data[index2], lag2_retail_data[index2])

# Aggregate monthly data to yearly sums or means for clearer plot
yearly_data <- tapply(retail_data, floor(time(retail_data)), sum)  # sum per year

# Plot yearly aggregated data
plot(names(yearly_data), yearly_data, type = "o",
     xlab = "Year", ylab = "Yearly Retail Sales (sum)",
     main = "Yearly Department Store Retail Sales",
     col = "black", pch = 16, lwd = 2)

decomposed <- decompose(retail_data) ##orignial time series, trend, seasonality and residual plots
plot(decomposed)

## Changing variance, seasonality, non-stationary
# checking if the original series follows normal distribution
check_normality(retail_data, "Figure 7: QQ plot of number of department stores in retail")
# Transformation 
...
plot(rt_dataBC, type = "o", ylab = "Box-cox transformed data of number of department stores in retail", xlab = "Time (Months)", main = "Figure 9: Box-Cox transformed data of number of department stores in retail")
# check noramality of transformed data
check_normality(rt_dataBC, "Figure 10: QQ plot of Box-cox transformed data of number of department stores in retail")
...
plot(rt_dataBCDiff, type = "o", ylab = "Number of department stores", xlab = "Time (Months)", main = "Figure 11: First Difference of Transformed data of number of department stores.")

# ACF & PACF plot of 1st difference on transformed series
acf(rt_dataBCDiff, main = "Figure 12: ACF of first difference of transformed data of number of department stores", xlab = "Lag", ylab = "ACF", lag.max = 80)
pacf(rt_dataBCDiff, main = "Figure 13: PACF of first difference of transformed data of number of department stores", xlab = "Lag", ylab = "PACF",ylim = c(-1, 1))
...
plot(rt_dataBCDiffSD, type = "o", ylab = "Number of department stores", xlab = "Time (Months)", main = "Figure 14: First & seasonal difference of transformed data of number of department stores.")

# ACF & PACF plot of 1st & seasonal difference on transformed series
acf(rt_dataBCDiffSD, main = "Figure 15: ACF of first & seasonal difference of transformed data of number of department stores", xlab = "Lag", ylab = "ACF", lag.max = 80)
pacf(rt_dataBCDiffSD, main = "Figure 16: PACF of first & seasonal difference of transformed data of number of department stores", xlab = "Lag", ylab = "PACF",ylim = c(-1, 1))
...
# ACF & PACF plot of 1st & seasonal difference on transformed series
acf(rt_dataBCDiffSD, main = "Figure 17: ACF of first & seasonal difference of transformed data of number of department stores", xlab = "Lag", ylab = "ACF", lag.max = 80)
pacf(rt_dataBCDiffSD, main = "Figure 18: PACF of first & seasonal difference of transformed data of number of department stores", xlab = "Lag", ylab = "PACF",ylim = c(-1, 1))
...
plot(res, main = "")
title(main = "Figure 14: BIC selection plot for models", line = 6)
